...
fn render() void {
    aya.gfx.beginPass();
    aya.gfx.texScale(checker_tex, 5, 5, 10);
    aya.gfx.texScale(checker_tex, 55, 55, 2);

    aya.gfx.texScale(font_tex, 200, 100, 0.2);
    aya.gfx.texScale(font_tex, 300, 200, 0.2);
    ...
    while (i <= @divFloor(rt.tex.width, checker_tex.width)) : (i += 1) {
        aya.gfx.tex(checker_tex, @intToFloat(f32, i * 4), @intToFloat(f32, i * 2 + 10));
    }
    aya.gfx.endPass();
    ...
    aya.gfx.texScale(rt.tex, 70, 200, 2);
    aya.gfx.endPass();
}
