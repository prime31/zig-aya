...
#line 34 ""
        float _54 = float(i);
        float _69 = v_63.time * 1000.0;
        float _74 = 500.0 + (_54 * 200.0);
        metaball_pos[i].x = ((sin(_69 / (_74 + (mod(_54, 6.0) * 500.0))) * 0.31847131252288818359375) + 0.5) * v_63.screen_size.x;
        metaball_pos[i].y = ((cos(_69 / (_74 - (mod(_54, 5.0) * 500.0))) * 0.31847131252288818359375) + 0.5) * v_63.screen_size.y;
        metaball_radius[i] = 150.0;
        metaball_influence[i] = 0.5;
    }
#line 41 ""
    float _128 = sin(v_63.time);
    metaball_pos[1].x = ((_128 * 0.31847131252288818359375) + 0.5) * v_63.screen_size.x;
    metaball_pos[1].y = ((cos(v_63.time * 0.25) * 0.31847131252288818359375) + 0.5) * v_63.screen_size.y;
#line 43 ""
    metaball_radius[1] = 400.0;
#line 44 ""
    metaball_influence[1] = 1.0;
    metaball_pos[2].x = ((sin(v_63.time * 0.5) * 0.31847131252288818359375) + 0.5) * v_63.screen_size.x;
    metaball_pos[2].y = ((cos(v_63.time * 0.3333333432674407958984375) * 0.31847131252288818359375) + 0.5) * v_63.screen_size.y;
#line 47 ""
...
#line 56 ""
    ...
    {
#line 58 ""
        float _199 = v_63.screen_size.y - gl_FragCoord.y;
        float _211 = gl_FragCoord.x + sin((_199 / dither_amount.x) + (cos(v_63.time) * 4.0));
        float _223 = _199 + cos((_211 / dither_amount.y) + (_128 * 4.0));
        float _323 = fast::min(1.0, ((((distance(float2(_211, _223), metaball_pos[i_1]) + pow((abs(metaball_pos[i_1].x - _211) * v_63.tear_sharpness) / abs(_223 - (metaball_pos[i_1].y - metaball_radius[i_1]) - (sin((_211 / v_63.tear_wave_length) + ((v_63.time * v_63.tear_wave_speed) / metaball_radius[i_1])) * v_63.tear_wave_amplitude)), 2.0)) + ((sin((_223 * 3.1400001049041748046875) + v_63.time) * metaball_radius[i_1]) / dither_amount.y)) + ((cos((_211 * 3.1400001049041748046875) + v_63.time) * metaball_radius[i_1]) / dither_amount.x)) / pow((fg.x * 0.89999997615814208984375) + 0.10000002384185791015625, 2.0)) / metaball_radius[i_1]);
