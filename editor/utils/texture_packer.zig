...
pub const TexturePacker = struct {
    pub const Atlas = struct {
        ...
        pub fn init(frames: []stb.stbrp_rect, files: [][]const u8, size: Size) Atlas {
            ...
            // generate the atlas
            var image = aya.Image.init(size.width, size.height);
            image.fillRect(.{ .w = size.width, .h = size.height }, aya.math.Color.transparent);

            for (files) |file, i| {
                var sub_image = aya.Image.initFromFile(file);
    ...
    pub fn pack(folder: []const u8) !Atlas {
        if (fs.cwd().openDir(folder, .{ .iterate = true })) |dir| {
            const pngs = aya.fs.getAllFilesOfType(aya.mem.allocator, dir, ".png", true);
    ...
    fn getFramesForPngs(pngs: [][]const u8) []stb.stbrp_rect {
        ...
        for (pngs) |png, i| {
            ...
            const tex_size = aya.Image.getTextureSize(png, &w, &h);
            frames.append(.{
                .id = @intCast(c_int, i),
                .w = @intCast(u16, w),
                .h = @intCast(u16, h),
