...
pub fn getModule(b: *std.Build, mach_core: *std.build.Module, enable_imgui: bool) *std.build.Module {
    const step = b.addOptions();
    step.addOption(bool, "enable_imgui", enable_imgui);

    return b.createModule(.{
        .source_file = .{ .path = thisDir() ++ "/src/zgui.zig" },
        .dependencies = &.{
            .{ .name = "options", .module = step.createModule() },
            .{ .name = "mach-core", .module = mach_core },
        },
    });
}
