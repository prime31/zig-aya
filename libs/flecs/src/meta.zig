...
pub fn componentId(world: *flecs.ecs_world_t, comptime T: type) u64 {
    ...
    if (@sizeOf(T) == 0) {
        ...
        type_id_ptr.* = flecs.c.ecs_entity_init(world, &desc);
    } else {
...
pub fn validateIterator(comptime Components: type, iter: *const flecs.ecs_iter_t) void {
    if (@import("builtin").mode == .Debug) {
        ...
        inline for (component_info.fields) |field| {
            ...
            if (is_const) assert(iter.terms[index].inout == flecs.EcsIn);
            if (iter.terms[index].inout == flecs.EcsIn) assert(is_const);
            ...
            if (is_optional) assert(iter.terms[index].oper == flecs.EcsOr or iter.terms[index].oper == flecs.EcsOptional or iter.terms[index - 1].oper == flecs.EcsOr);
            if (iter.terms[index].oper == flecs.EcsOr or iter.terms[index].oper == flecs.EcsOptional) assert(is_optional);
    ...
    if (@import("builtin").mode == .Debug) {
        ...
        if (@hasDecl(Components, "modifiers")) {
            inline for (Components.modifiers) |inout_tuple| {
                ...
                if (ti.inout == flecs.EcsInOutFilter) {
                    if (ti.term_type == T)
                        valid = true;
                }
            }
        ...
        .Struct => |si| {
            ...
            inline for (si.fields, 0..) |field, i| {
                ...
                member.type = switch (field.type) {
                    ...
                    bool => flecs.FLECS_IDecs_bool_tID_,
                    f32 => flecs.FLECS_IDecs_f32_tID_,
                    f64 => flecs.FLECS_IDecs_f64_tID_,
                    u8 => flecs.FLECS_IDecs_u8_tID_,
                    u16 => flecs.FLECS_IDecs_u16_tID_,
                    u32 => flecs.FLECS_IDecs_u32_tID_,
                    u64 => blk: {
                        ...
                        if (std.mem.indexOf(u8, field.name, "entity") != null)
                            break :blk flecs.FLECS_IDecs_entity_tID_;
                        break :blk flecs.FLECS_IDecs_u64_tID_;
                    },
                    i8 => flecs.FLECS_IDecs_i8_tID_,
                    i16 => flecs.FLECS_IDecs_i16_tID_,
                    i32 => flecs.FLECS_IDecs_i16_tID_,
                    i64 => flecs.FLECS_IDecs_i64_tID_,
...
pub fn generateFilterDesc(world: flecs.World, comptime Components: type) flecs.ecs_filter_desc_t {
    ...
    inline for (component_info.fields, 0..) |field, i| {
        desc.terms[i].id = world.componentId(FinalChild(field.field_type));

        if (@typeInfo(field.field_type) == .Optional)
            desc.terms[i].oper = flecs.EcsOptional;

        if (isConst(field.field_type))
            desc.terms[i].inout = flecs.EcsIn;
    }
    ...
    if (@hasDecl(Components, "modifiers")) {
        inline for (Components.modifiers) |inout_tuple| {
            ...
            if (getTermIndex(ti.term_type, ti.field, &desc, component_info.fields)) |term_index| {
                ...
                if (ti.inout == flecs.EcsInOutNone) {
                    assert(ti.oper == flecs.EcsOr);
                    ...
                    if (ti.inout == flecs.EcsOut) {
                        assert(desc.terms[term_index].inout == flecs.EcsInOutDefault);
                ...
                // the term wasnt found so we must have either a InOutNone, Not or EcsNothing mask
                if (ti.inout != flecs.c.EcsInOutNone and ti.oper != flecs.c.EcsNot) std.debug.print("invalid inout found! No matching type found in the Components struct. Only Not and Filters are valid for types not in the struct. This should assert/panic but a zig bug lets us only print it.\n", .{});
                if (ti.inout == flecs.c.EcsInOutNone) {
                    desc.terms[next_term_index].id = world.componentId(ti.term_type);
                    desc.terms[next_term_index].inout = ti.inout;
                    next_term_index += 1;
