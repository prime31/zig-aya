...
pub const Renderer = struct {
    ...
    pub fn init(docking: bool, viewports: bool, icon_font: bool) Renderer {
        var enable_viewports = viewports;
        if (enable_viewports) {
            enable_viewports = false;
            std.debug.print("imgui viewports were enabled but they are not yet supported\n", .{});
        }
        ...
        if (enable_viewports) io.ConfigFlags |= imgui.ImGuiConfigFlags_ViewportsEnable;
