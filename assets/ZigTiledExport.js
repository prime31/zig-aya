...
var zigMapFormat = {
	...
	getTileset: function(tileset) {
		var ts = {
			...
			image: zigMapFormat.cleanImagePath(tileset.image),
			image_columns: zigMapFormat.getPngColumnCount(tileset),
			tiles: []
		};
		...
		for (var j = 0; j < tileset.tiles.length; j++) {
			...
			if (tileHasData) {
				tiled.warn(`adding tileset tile: (tileId => index) ${tile.id} => ${Object.keys(zigMapFormat.tilesetTiles).length}`);
				zigMapFormat.tilesetTiles[tile.id] = Object.keys(zigMapFormat.tilesetTiles).length;
				ts.tiles.push({
					id: tile.id,
					props: props
				});
	...
	getTileLayer: function(layer) {
        ...
        for (var y = 0; y < layer.height; y++) {
            for (var x = 0; x < layer.width; x++) {
				var id = zigMapFormat.getTileId(layer.cellAt(x, y), layer.flagsAt(x, y));
				l.tiles.push(id);
            }
        }
	...
	getGroupLayer: function(layer) {
		...
		for (var i = 0; i < layer.layerCount; i++) {
			...
			if (subLayer.isTileLayer) {
				l.tile_layers.push(zigMapFormat.getTileLayer(subLayer));
				...
				l.object_layers.push(zigMapFormat.getObjectLayer(subLayer));
			}
		}
    ...
    write: function(map, fileName) {
		...
		for (var i = 0; i < tilesets.length; i++) {
			...
			m.tilesets.push(zigMapFormat.getTileset(tileset));
		}
        ...
        for (var i = 0; i < map.layerCount; i++) {
            ...
            if (layer.isTileLayer) {
				m.tile_layers.push(zigMapFormat.getTileLayer(layer));
            } else if (layer.isObjectGroup) {
				throw 'layer.isObjectGroup is true. This wasnt previously possible.';
			} else if (layer.isImageLayer) {
				tiled.warn('ImageLayers not supported. Ignoring.');
			} else if (layer.isGroupLayer) {
				m.group_layers.push(zigMapFormat.getGroupLayer(layer));
			} else {
				m.object_layers.push(zigMapFormat.getObjectLayer(layer));
			}
        }
		...
		for (var i = 0; i < zigMapFormat.images.length; i++) {
			var src = zigMapFormat.images[i];
...
tiled.registerMapFormat('V', zigMapFormat)

var action = tiled.registerAction('Export Zig', function(action) {
    tiled.trigger('ExportAs');
})

action.text = 'Export Zig Map'
action.shortcut = 'Ctrl+K'

tiled.extendMenu("Edit", [
    { action: 'Export Zig', before: 'SelectAll' },
    { separator: true }
]);
